            ...
            <tbody >
                ...
                <tr  v-for="(combo, comboIdx) in store.allCombinationListData" :key="comboIdx">
                    <td class="bg-opacity-50 text-center" :class="getRowStyleByRank(combo.rank)">
                        <input type="radio" :value="combo" 
                            id="irrigationCombination" v-model="store.solutionUserPicked.irrigationCombination">
                    </td>
                    <td class="bg-opacity-50 text-center" :class="getRowStyleByRank(combo.rank)">{{ (combo.rank > 0) ? combo.rank :
                        (combo.rank + 1) }}</td>
                    <td class="bg-opacity-50 text-center" :class="getRowStyleByRank(combo.rank)">
                        {{ combo.title }}
                    </td>
...
.table-container {
    max-height: 610px; /* 限制最大高度，讓內容滾動 */
    overflow-y: auto; /* 垂直滾動 */
    border: 1px solid #ddd; /* 加上邊框以區分表格 */
}
.table {
    width: 100%;
...
thead {
    position: sticky;
    top: 0;
    background: white; /* 確保固定時表頭有背景色 */
    z-index: 10; /* 讓表頭層級較高，避免被內容蓋住 */
}
...
tbody tr:hover {
    background-color: rgba(0, 0, 0, 0.05); /* 滑鼠移上時的背景變化 */
}
</style>